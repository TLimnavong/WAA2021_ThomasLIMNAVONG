...
function registerUser(){
    ...
    let saveImage_btn=document.getElementById('screenCanvas')
    ...
    if(input.value.length >= 3){
        ...
        if(draw_btn.disabled == false && saveImage_btn.disabled==false){ draw_btn.disabled = true }{saveImage_btn.disabled = true }
            username = input.value
...
function screenImage(){
    let screencanvas = canvas.toDataURL()
    
    let newwindow= window.open('about:blank');// because when we open the canvas in the new window the page name is "about:blank"
    let canva = new Image();
    canva.src = screencanvas;
    
    newwindow.document.write(canva.outerHTML);
