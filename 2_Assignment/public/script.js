...
function registerUser(){
    ...
    if(input.value.length >= 3){
        ...
        if(draw_btn.disabled == false){ draw_btn.disabled = true }{saveImage_btn.disabled = true }
            username = input.value
...
function saveImage(){
    ...
    const d= new Date()
    var date=d.getTime()
    let savecanvas = canvas.toDataURL()
    ...
    {   
        ...
        method: 'POST',
        body:JSON.stringify({username: username ,nowdate: date, image: savecanvas })
    })
